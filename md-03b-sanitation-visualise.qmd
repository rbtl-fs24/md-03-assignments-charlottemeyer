...
```{r}
sanitation_Nepal_2020_2022_without_sm <- sanitation |> 
  filter(name == "Nepal", year %in% c(2000, 2020), varname_long != "safely managed sanitation services")
```
...
```{r}
sanitation_Nepal_2020_2022_without_sm |> 
  count(year)
```
...
```{r}
sanitation_Nepal_2020_2022_without_sm |> 
  ggplot(mapping = aes(x = residence, y = percent, fill = varname_long)) +
  geom_col()+
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend? -\> they are in alphabetical order
2.  What would you want to change? -\> national data on the 3rd column, and arrange legend order to "no", "unimproved", "limited" and "basic" for example
3.  Why did we remove "safely managed sanitation services" from the data set in Task 3? -\> I am not sure, but I guess to have to number adding up to 100%

## Task 7

1.  Run the code in the code chunk below.
2.  What do you observe when you look at the code and plot? -\> the data is plotted as histograms for the number of countries that has each of the sanitation conditions, and compared in the different residence
